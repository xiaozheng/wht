...
function WPWall_ShowComments($page = 1) {
	...
	if ( $comments ) {  
		...
		foreach ($comments as $comment) {			
			...
			$comment_content="";
			$comment_heading="";
			$comment_heading_value=explode("csheadingContentSeparatorcs",$comment->comment_content);
			if(count($comment_heading_value) >1)
			{
			$comment_content_1=$comment_heading_value[1];
			$comment_heading=$comment_heading_value[0];	
			}else{
			$comment_content_1=$comment_heading_value[0];
			$comment_heading=$comment_heading_value[0];
			}// $deleteurl it will give comment delete  link
			$result.= '<p class="wall-'.$count.$oddcomment.' '.$highlight.'">'.WPWall_Gravatar($comment).
			'<span class="wallHeading">'.$comment_heading .'</span><br/>'.
			'<span class="wallauthor" title="'.htmlspecialchars($comment->comment_author).' on '.$comment->comment_date.'" >' 
			.date($dateoption, strtotime($comment->comment_date)).' | '.htmlspecialchars($comment->comment_author).'</span><br />
			
			<span class="wallcomment" title="'.htmlspecialchars(strip_tags($comment_content_1)).'" > '.
			convert_smilies(WPWall_OneWordwrap(WPWall_GetExcerpt($comment_content_1, $options['wall_wrap']), $wordwrap)).
			'</span></p>';
			$count++;
...
<div class="wrap" style="">
	...
	<div id="sideblock" style="float:right;width:270px;margin-left:10px;"> 
		 <iframe width=270 height=800 frameborder="0" src="http://www.prelovac.com/plugin/news.php?id=3&utm_source=plugin&utm_medium=plugin&utm_campaign=WP%2BWall"></iframe>
 	</div>
